...
test_images_results = list(map(lambda x: _process_frame(mpimg.imread(x)), test_images))
fig, axs = plt.subplots(3, 2, figsize=(16, 14))
axs = axs.ravel()

for i in range(len(test_images_results)):
    axs[i].imshow(test_images_results[i])
    axs[i].axis('off')
...
print("Video1...")
test_out_file = 'test_video_out_basic.mp4'
clip_test = VideoFileClip('test_video.mp4')
clip_test_out = clip_test.fl_image(_process_frame)
clip_test_out.write_videofile(test_out_file, audio=False)

print("Video2...")
proj_out_file = 'project_video_out_basic.mp4'
clip_proj = VideoFileClip('project_video.mp4')
clip_proj_out = clip_proj.fl_image(_process_frame)
